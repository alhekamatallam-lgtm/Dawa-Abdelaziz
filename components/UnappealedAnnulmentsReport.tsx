...
const UnappealedAnnulmentsReport: React.FC<UnappealedAnnulmentsReportProps> = ({ sessions, onSessionClick }) => {
    ...
    // 1. Identify all violation numbers that have an appeal case
    const appealViolationNumbers = useMemo(() => {
        ...
        sessions.forEach(s => {
            ...
            if (caseType.includes('استئناف') && violationNo !== '') {
                set.add(violationNo);
            }
        });
        return set;
    ...
    const allInitialAnnulledSessions = useMemo(() => {
        return sessions.filter(s => {
            ...
            const caseType = (s['نوع الدعوى'] || '').toString().trim();
            // Exclude appeal sessions
            if (caseType.includes('استئناف')) return false;
    ...
    // 3. Filter for unappealed annulments
        ...
        allInitialAnnulledSessions.forEach(s => {
            ...
            if (vNo !== '') {
                ...
                const isAppealed = sessions.some(sec => 
                    (sec['نوع الدعوى'] || '').toString().includes('استئناف') && 
    ...
    }, [unappealedAnnulledSessions, selectedCourt, selectedPlaintiff, searchQuery]);
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-300">
            ...
            <div className="bg-white p-5 rounded-2xl border border-border shadow-sm space-y-4 print:hidden">
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <input 
                                ...
                                placeholder="ابحث برقم الدعوى، رقم المخالفة، المدعي، أو المحامي..."
                ...
                {(searchQuery || selectedCourt !== 'all' || selectedPlaintiff !== 'all') && (
                    <div className="flex items-center justify-between pt-2 border-t border-border/50 text-xs">
                        <span className="text-dark/60 font-medium">
                            عدد النتائج المطابقة: <strong className="text-emerald-700">{filteredSessions.length}</strong> من أصل {unappealedAnnulledSessions.length}
                        </span>
                        <button 
                            onClick={() => {
                                setSearchQuery('');
                        ...
                        <table className="w-full text-right border-collapse">
                            <thead>
                                <tr className="bg-emerald-900 text-white text-xs font-bold">
                                    ...
                                    <th className="p-4 text-center">حالة الاستئناف</th>
                                ...
                                {filteredSessions.map((session, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="p-4 text-center">
                                                <span className="inline-block px-2.5 py-1 bg-rose-50 text-rose-700 font-bold text-[10px] rounded-md border border-rose-200">
                                                    لم يتم الاستئناف
                                                </span>
                                            </td>
